...
//    "if","else","void","return","while","then","for","do","int","long","char","double","float","case","cin","cout"
//};  //关键字字典
//
//int state=0;    //初始化状态为0
//void stat() {
//
//}   //统计函数
//void analyze(FILE* fpoint) {
//    char C = ' ';   //创建字符变量，用于存放当前读到的字符
//    string token="";    //存放当前读入字符串
//    while (C != EOF)/*循环到文件末尾*/ {
//        switch (state) {
//        case 0:
//            token = ""; //清空缓冲区
//            while (!isspace(C)) {   //读入一个非空格字符
//                C = fgetc(fpoint);
//            }
//            if (isalpha(C))/*C为字母*/ {
//                state = 1;
